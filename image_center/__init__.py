...
class ImageCenter:
    ...
    @classmethod
    def image_center(cls, *widget, rate: [float, int] = None, multiple: bool = False, match_number: int = None):
        """
         在屏幕中区寻找小图，返回坐标，
    ...
    @staticmethod
    def opencv_compare_images_SSIM(module_name, imageA_name, imageB_name, ssim=None, app_name=None):
        """
        判断两张图片是否相同
        ...
        """
        if ssim is None:
            ssim = setting.IMAGE_RATE
        picture_path = f"{setting.APPS_PATH}/{app_name}/res/picture"
        contrast_path = f"/home/{setting.USERNAME}/Pictures/{app_name}"
